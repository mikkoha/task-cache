...
public interface ITaskCache
{
    ...
    /// <param name="options">Allows customizing expiration settings if a value for the given key is not already present in the cache.</param>
    /// <returns>Returned task-object can be completed or running. Note that the task might result in an exception.</returns>
    Task<T> AddOrGetExisting<T>(string key, Func<Task<T>> valueFactory, TaskCacheEntryOptions options = default);
